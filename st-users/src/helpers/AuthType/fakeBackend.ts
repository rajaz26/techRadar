...
const fakeBackend = () => {
  ...
  mock.onPost(url.ADD_INVOICE).reply(invoice => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (invoice.data) {
          // Passing fake JSON data as response
  ...
  mock.onDelete(url.DELETE_INVOICE).reply(headers => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (headers && headers.headers) {
          // Passing fake JSON data as response
          resolve([200, headers.headers.invoice]);
        } else {
  ...
  mock.onDelete(url.DELETE_USER).reply(headers => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (headers && headers.headers) {
          // Passing fake JSON data as response
          resolve([200, headers.headers.user]);
        } else {
          reject([400, "Cannot delete invoice"]);
        }
      });
